...
shinyServer(function(input, output, session) {
    ...
    output$populares <- renderPlot({
        ...
        # print(programa_df)
        # print(str(programa_df$Programa))
        ...
        # Also this one for ymax
        
        ggplot(programa_df, aes(x = Programa,y = count, ymax = 1.1*max(count))) + geom_bar(stat = "identity", fill=color_blind_palette[1],position = "dodge", width = 0.8) + geom_text(aes(label = Avg_IGS, x = Programa, y = count), position = position_dodge(width = 0.8), vjust = -0.6, colour = "red") + theme(axis.text.x = element_text(angle = 60, hjust = 1))
