...
shinyServer(function(input, output, session) {
    ...
    output$selectivos <- renderPlot({
        ...
        selective_plot <- base_plot + geom_bar(stat = "identity", position = "dodge", width = 0.8) + geom_text(aes(label = paste("IGS =",Avg_IGS), x = Programa, y = count), position = position_dodge(width = 0.75), vjust = -0.6, colour = "red") 
