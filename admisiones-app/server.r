...
shinyServer(function(input, output, session) {
    ...
    output$DataTable <- renderDataTable({
        # Fetch relevant data
        relevant_data <- filter_data()
        relevant_data <- rename(relevant_data, replace=c("Institucion" = "Institución"))
        relevant_data <- rename(relevant_data, replace=c("Genero" = "Género"))
        
        # Use relevant columns for table
        relevant_data_table <- relevant_data[c("Género", "Institución", "Programa", "GPA", "IGS")]
        relevant_data_table
